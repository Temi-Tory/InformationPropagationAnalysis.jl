    ...
    # Final identification of conditioning nodes
    targets_in_final = Set{Int64}()
    for (_, target) in final_edgelist
        push!(targets_in_final, target)
    end
...
function perform_fresh_diamond_lookup(
    ...
    # REWRITE FIX #1: Always compute fresh diamonds with full context-keyed cache
    # This eliminates Bug #1 (hybrid-reuse context-blindness) by ensuring
    # every unique context gets fresh computation

    stats = OptimizationStats(0, 0, 0, 0, 0.0)

    # Always compute fresh for current join
    fresh_diamonds_dict = identify_and_group_diamonds(
        ...
        Dict{Tuple{Set{Int64}, Set{Int64}, UInt64}, Dict{Int64, DiamondsAtNode}}(),  # Fresh cache per call
        ctx
    )

    # Update stats
    total_joins = length(sub_join_nodes)
    stats = OptimizationStats(total_joins, length(fresh_diamonds_dict),
                            length(fresh_diamonds_dict), length(fresh_diamonds_dict), 100.0)

    return fresh_diamonds_dict, stats
end
...
function build_unique_diamond_storage(
    ...
    while !isempty(work_stack)
        ...
        if is_root_diamond
            ...
            # SUB DIAMONDS: Use fresh computation (not hybrid reuse - eliminates Bug #1)
            sub_diamonds_dict, optimization_stats = perform_fresh_diamond_lookup(
